...
<?php foreach ($raices as $raiz_item): ?>
	<tr>
		...
		<?php if($opcion_delete) { ?><td><a href="/<?php echo DIR_SIIGS; ?>/raiz/delete/<?php echo $raiz_item->id ?>"  class="btn btn-primary"onclick="if (<?php echo $raiz_item->catalogos?>=='0'){if (confirm('¿Realmente desea eliminar esta raíz?')) { return true; } else {return false;}} else {alert('No se puede eliminar la raíz porque ya contiene catálogos asociados.') ; return false;}">Eliminar<i class="icon-remove"></i></a></td><?php } ?>
	</tr>
<?php endforeach ?>
<?php } else {?>
<thead>
