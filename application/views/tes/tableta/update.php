...
if(!empty($msgResult)) {
    ...
    <?php echo form_open(site_url().DIR_TES.'/tableta/update/'.$registro->id); ?>
        ...
        <tr><td colspan="2">
            <button type="submit" class="btn btn-primary">Actualizar <i class="icon-hdd"></i></button>
            <button type="button" name="cancelar" onclick="location.href='<?php echo site_url().DIR_TES; ?>/tableta/'" class="btn btn-primary">Cancelar <i class="icon-arrow-left"></i></button>
        </td></tr>
        </table>
        </div>
    </form>
