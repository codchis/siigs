...
function isDate(dtStr){
	...
	year=parseInt(strYr)
	if (pos1==-1 || pos2==-1){
        showAlerta({mensaje: 'El formato de la fecha debe ser dd/mm/yyyy'});
		return false
	}
	if (strMonth.length<1 || month<1 || month>12){
        showAlerta({mensaje: 'El mes es incorrecto'});
		return false
	}
	if (strDay.length<1 || day<1 || day>31 || (month==2 && day>daysInFebruary(year)) || day > daysInMonth[month]){
        showAlerta({mensaje: 'El día es incorrecto'});
		return false
	}
	if (strYear.length != 4 || year==0){
        showAlerta({mensaje: 'El año es incorrecto'});
		return false
	}
